...
export default function NodedraggingFactory (modelservice, nodeDraggingScope, applyFunction, automaticResize, dragAnimation) {
  ...
  return {
    dragstart: function (node, event) {
      ...
      var elements = []
      var nodes = []
      if (store.flow.getters.isSelectedObject(node)) {
        var selectedNodes = store.flow.getters.getSelectedNodes()
        for (let i = 0; i < selectedNodes.length; i++) {
          var selectedNode = selectedNodes[i]
          var element = store.flow.state.nodeElements[selectedNode.id]
      ...
      for (let i = 0; i < elements.length; i++) {
        let element = elements[i].getBoundingClientRect()
        offsetsX.push(parseInt(element.left) - event.clientX)
        offsetsY.push(parseInt(element.top) - event.clientY)
      }
      if (modelservice.isDropSource()) {
        ...
        var offset = modelservice.getCanvasHtmlElement().getBoundingClientRect()
    ...
    dragover: function (event) {
      ...
      if (dragAnimation === flowchartConstants.dragAnimationRepaint) {
        if (nodeDraggingScope.draggedNodes.length) {
          return applyFunction(function () {
            for (let i = 0; i < nodeDraggingScope.draggedNodes.length; i++) {
              var draggedNode = nodeDraggingScope.draggedNodes[i]
              var dragOffset = dragOffsets[i]
