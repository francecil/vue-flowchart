        ...
        return function (event) {
          ...
          if (modelservice.nodes.isSelected(node)) {
            ...
            for (var i = 0; i < selectedNodes.length; i++) {
              ...
              var element = jquery(modelservice.nodes.getHtmlElement(selectedNode.id))
              elements.push(element)
              nodes.push(selectedNode)
            }
          } else {
            elements.push(jquery(event.target))
            nodes.push(node)
          }
          var offsetsX = []
          var offsetsY = []
          ...
          if (modelservice.isDropSource()) {
            ...
            if (event.originalEvent.dataTransfer.setDragImage) {
              // var invisibleDiv = jquery('<div></div>')[0]; // This divs stays invisible, because it is not in the dom.
          ...
          if (dragAnimation === flowchartConstants.dragAnimationShadow) {
            for (let i = 0; i < draggedElements.length; i++) {
              ...
              var shadowElement = jquery('<div style="position: absolute; opacity: 0.7; top: ' + getYCoordinate(dragOffset.y + event.clientY) + 'px; left: ' + getXCoordinate(dragOffset.x + event.clientX) + 'px; "><div class="innerNode"><p style="padding: 0 15px;">' + draggedNode.name + '</p> </div></div>')
              var targetInnerNode = jquery(draggedElements[i]).children()[0]
